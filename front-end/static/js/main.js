  ...
  if (filterProperty === "categories") {
    ...
    data = data.filter((recipe) =>
      recipe.ingredients.some(
        (ingredient) =>
          ingredient.name.toLowerCase() === filterData.toLowerCase()
      )
    );
...
async function POSTRecipe() {
  // const $btn = document.getElementById("New_Recipe_btn");
  ...
  $form.addEventListener("submit", async () => {
    ...
    await fetch(API_URL, {
      ...
      .then((response) => (response = $answer.innerHTML = response.message))
